...
app.post("/GlobeStatus/currentWeatherByName",function(req,res){
    ...
    xhr.onload = function(){
        if(this.status==200){
            ...
            res.render("showCurrentWeather.ejs",{ lon, lat, cityNameDisplay, imgUrl, minTemp, maxTemp
            , temp, pressure, humidity, desc, windSpeed, windDir, datesArr,
    ...
    xhr2.onload = function(){
        if(this.status==200)
        {
            ...
            else
            {
                ...
                imgUrl = imageData.results[random].urls.small;
                const datesArr = getNextFiveDays();
                res.render("showCurrentWeatherByCoordinates",{ lon, lat, cityName, windSpeed, windDir, 
...
app.post("/GlobeStatus/weatherAlerts/:cityName/:lat/:lon/:date", function(req,res){
    ...
    xhr.onload = function(){
        if(this.status == 200)
        {
            ...
            if(data && data.list) {
                ...
                res.render("showWeatherAlerts", 
                    { dateSpecificData, pagesPerDate, lat, lon, cityName }
                );
            }
        } else {
